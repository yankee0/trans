    ...
    <main>
      ...
      <ul>
        <?php foreach ($tcs as $tc) : ?>
          <li><strong><?= $tc['designation'] ?></strong> du TC <strong class="red"><?= $tc['conteneur'] ?> <?= $tc['type'] ?>'</strong> pour le client <strong><?= $tc['client'] ?></strong>. Pregate reçu le <strong><?= date('d/m/Y', strtotime($tc['date_creation'])) ?></strong> et <strong><?= $tc['paiement'] == 'OUI' ? 'payé le ' . $tc['date_paiement'] : 'NON PAYÉ.' ?></strong>
          <?php endforeach ?>
      </ul>
